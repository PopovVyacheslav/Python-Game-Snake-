...
class Game():
    ...
    def show_score(self, choice=1):
        ...
        #Дефолтный случай: слева сверху
        if choice == 1:
            s_rect.midtop = (80, 10)
        #иначе у нас game_over и отображаем по центру
        else:
    ...
    def game_over(self):
        ...
        go_surf = go_font.render('Game over', True, 72, self.red)
        go_rect = go_surf.get_rect()
        go_font.midtop = (360, 15)
...
class Snake():
    def __init__(self, snake_color):
        ...
        #направление движения змеи, изначально вправо
    ...
    def snake_body_mechanism(
            self, score, food_pos, screen_width, scree_height):
        self.snake_body.insert(0, list(self.snake_head_pos))

        if (self.snake_head_pos[0] == food_pos[0] and
                self.snake_head_pos[1] == food_pos[1]):
            food_pos = [random.randrange(1, screen_width / 10) * 10,
                        random.randrange(1, scree_height / 10) * 10]
            score += 1
        else:
            self.snake_body.pop()
